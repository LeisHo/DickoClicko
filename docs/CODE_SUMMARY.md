...
- `index.html` -- the entire app. One `<canvas>` for the animation, one
  `<div id="devPanel">` (built dynamically from a config array) for the dev
...
```
DEV_GROUPS (config array: one entry per *DEV*/*COL* from the spec)
...
update(rawDt) each animation frame:
    ...
    3. integrateChain() for mainRope
    4. integrateChain() for every fallenPieces entry (its own gravity scale)
...
circle (circle drawn last so the rope visually emerges from behind it).
```
...
- A perfectly vertical cut piece has no asymmetry to fall over on, so
  ...
  piece's own cut-point end) before it's added to `fallenPieces` -- without
  this, a piece cut from an undisturbed hanging rope stands on the floor as
  a rigid straight column instead of settling into a heap. Confirmed via
  direct physics-state inspection during this build (see CHANGELOG).
